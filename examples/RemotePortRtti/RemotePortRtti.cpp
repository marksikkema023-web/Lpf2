...
void loop()
{
    ...
    if (hub.isConnected())
    {
        ...
        {
            if (auto device = dynamic_cast<Lpf2::Devices::TechnicColorSensorControl *>(dev))
            {
                Serial.print("Color idx: ");
                Serial.println(device->getColorIdx());
            }
            else if (auto device = dynamic_cast<Lpf2::Devices::EncoderMotorControl *>(dev))
            {
                if (firstTime)
                {
            ...
            else if (auto device = dynamic_cast<Lpf2::Devices::BasicMotorControl *>(dev))
            {
